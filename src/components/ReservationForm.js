...
class ReservationForm extends Component {
  ...
  addNewRes = () => {
    ...
    .then(response => {
      ...
      this.setState({
        status: response.statusText === "Created" ? "Reservation complete. Thank you for choosing Karachi Airlines." : ""
      });
    })
    .catch(error => {
      // console.log( error.message === "Request failed with status code 422" ? "Sorry, that seat is unavailable." : "" );
      this.setState({
        status: error.message === "Request failed with status code 422" ? "Sorry, that seat is unavailable. Please choose an available seat or call 777 Karachi Airlines for assistance." : ""
      });
    })
  }
