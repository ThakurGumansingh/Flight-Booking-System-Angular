...
class ExistingRes extends Component {
  ...
  render() {
    ...
    if (Object.keys(this.props.res).length!== 0){
      ...
      if (fio.length > 0){
        return (
          <div style={{backgroundColor: 'indianred', color: 'InactiveBorder'}}>
            <h4>
            <p style={{color: 'blue'}}>MY RESERVATIONS</p>
            </h4>
            {fio.map (s=>
              <div key={s.id} className="flex-container">
                ...
                <button style={{backgroundColor: 'rgb(105, 237, 100)', color: 'black'}}className="res-button book cancel flex-1" onClick = { () => { this._handleClick(s,fio) } }>Cancel Reservation</button>
              </div>
